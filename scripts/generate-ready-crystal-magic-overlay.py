#!/usr/bin/env python3
"""Generate a deterministic candidate MagicInfo overlay for runtime-ready spells.

The candidate is research output only until CI proves it can be applied/rebuilt.
Existing Zircon MagicInfo rows are matched by normalized display name, keeping
Magic/Class/School/Property/Description and the existing Index unchanged.
"""
...
def main() -> int:
    ...
    for decision in decisions["decisions"]:
        ...
        if numeric.get("status") != "projected_by_name":
            raise RuntimeError(f"Numeric projection for {spell} is not a verified name projection: {numeric.get('status')}")

        matches = zircon_by_name.get(key, [])
        if len(matches) != 1:
            raise RuntimeError(f"Expected exactly one Zircon MagicInfo name match for {spell}; found {len(matches)}")
        ...
        included.append({
            ...
            "zirconIndex": target["Index"],
            "zirconName": target.get("Name"),
            "zirconMagic": target.get("Magic"),
    ...
    payload = {
        "SchemaVersion": 1,
        "Name": "Runtime-ready Crystal spells mapped into existing Zircon MagicInfo rows",
        ...
        "$audit": {
            ...
            "existingZirconMagicClassSchoolPropertyPreserved": True,
            "included": included,
            "skipped": skipped
        }
    }
    ...
    print(f"Ready Crystal magic overlay candidate: {len(operations)} updates, {len(skipped)} skipped")
    for item in included:
        print(
            f"- {item['crystalSpell']} -> Zircon MagicInfo#{item['zirconIndex']} "
            f"({item['zirconName']})"
        )
    return 0
