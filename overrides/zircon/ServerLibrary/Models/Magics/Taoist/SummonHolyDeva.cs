...
{
    [MagicType(MagicType.SummonHolyDeva)]
    public sealed class SummonHolyDeva : MagicObject
    {
        protected override Element Element => Element.None;
        public override bool UpdateCombatTime => false;

        public SummonHolyDeva(PlayerObject player, UserMagic magic) : base(player, magic) { }

        public override MagicCast MagicCast(MapObject target, Point location, MirDirection direction)
        {
            var response = new MagicCast { Ob = null };
            MonsterInfo info = SEnvir.MonsterInfoList.Binding.FirstOrDefault(x => x.MonsterName == "HolyDeva");
            if (info == null)
            {
                response.Cast = false;
                return response;
            ...
            if (Player.Pets.Count(x => x.Race == ObjectType.Monster && !x.Dead) >= 2 || !Player.UseAmulet(2, 0))
            {
                response.Cast = false;
                return response;
            }
