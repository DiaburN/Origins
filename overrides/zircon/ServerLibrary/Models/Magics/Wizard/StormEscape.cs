...
{
    [MagicType(MagicType.StormEscape)]
    public sealed class StormEscape : MagicObject
    {
        protected override Element Element => Element.Lightning;

        public StormEscape(PlayerObject player, UserMagic magic) : base(player, magic)
        {
        }
        ...
        public override void MagicComplete(params object[] data)
        {
            ...
            if (SEnvir.Random.Next(4) >= Magic.Level + 1) return;
            if (!Player.Teleport(castMap, location, false)) return;

            var stats = new Stats
            {
                [Stat.TeleportManaPenaltyPercent] = 30
            };
        ...
        {
            // Crystal trains StormEscape once for its damage phase; teleport success can level it again.
        }
