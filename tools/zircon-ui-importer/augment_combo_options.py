...
Promotes only source-backed options whose visible label is a C# string literal or
resolved CEnvir.Language key. Handles both direct DXListBoxItem initializers and
simple helper methods such as GameStoreDialog.AddSortOption(...). Runtime/database
options remain intentionally absent.
"""
...
def label_from_expression(expression: str, messages: dict[str, str]) -> tuple[str, str] | None:
    ...
    language = re.fullmatch(r"CEnvir\.Language\.([A-Za-z_][A-Za-z0-9_]*)", expression)
...
def helper_options(body: str, combo_names: set[str], messages: dict[str, str]) -> dict[str, list[dict]]:
    ...
    for method in method_re.finditer(body):
        ...
        for initializer in list_item_initializers(method_body):
            ...
            calls = re.finditer(rf"\b{re.escape(method.group(1))}\s*\((.*?)\)\s*;", body, re.S)
            for call in calls:
                ...
                entry = {"label": label[0], "labelSource": label[1], "valueExpression": " ".join(value_expression.split()) if value_expression else None, "sourceBuilder": method.group(1)}
                bucket = found[parent.group(1)]
                if not any(existing["label"] == entry["label"] and existing.get("valueExpression") == entry.get("valueExpression") for existing in bucket):
                    bucket.append(entry)
    return found
...
def main() -> None:
    ...
    combos = 0; with_options = 0; option_count = 0; selected_count = 0; helper_option_count = 0
    ...
    for owner in [*(spec.get("windows") or []), *(spec.get("nestedWindows") or [])]:
        ...
        for initializer in list_item_initializers(body):
            ...
            entry = {"label": label[0], "labelSource": label[1], "valueExpression": " ".join(item.group(1).split()) if item else None}
            merge_entries(found[parent.group(1)], [entry])
    ...
    spec["comboOptionPass"] = {
        ...
        "source": "direct/helper-built DXListBoxItem options with literal/resolved-language labels",
    }
    args.spec.write_text(json.dumps(spec, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(f"DXComboBox deterministic options: {option_count} across {with_options}/{combos}; helper-built={helper_option_count}; selected={selected_count}")
