...
CASES=(
    # field, source file, type, source evidence proving runtime creation
    ('CommunicationBox','Client/Scenes/Views/CommunicationDialog.cs','FriendRow',(
        'foreach (ClientFriendInfo info in FriendList.OrderBy(x => x.State))',
        'FriendListBoxItems.Add(row = new FriendRow()',
    )),
    ('HelpBox','Client/Scenes/Views/HelpDialog.cs','HelpContainer',(
        'foreach (var helpInfo in Globals.HelpInfoList.Binding.OrderBy(x => x.Order))',
        'var page = new HelpContainer(info)',
    )),
    ('HelpBox','Client/Scenes/Views/HelpDialog.cs','HelpItem',(
        'foreach (var infoSection in infoPage.Items.OrderBy(x => x.Order))',
        'HelpItem cell = new HelpItem',
    )),
    ('QuestBox','Client/Scenes/Views/QuestDialog.cs','QuestTreeEntry',(
        'foreach (QuestInfo quest in pair.Value)',
        'QuestTreeEntry entry = new QuestTreeEntry',
    )),
    ('CurrencyBox','Client/Scenes/Views/CurrencyDialog.cs','CurrencyTreeHeader',(
        'foreach (KeyValuePair<string, List<ClientUserCurrency>> pair in TreeList)',
        'CurrencyTreeHeader header = new CurrencyTreeHeader',
    )),
    ('CurrencyBox','Client/Scenes/Views/CurrencyDialog.cs','CurrencyItem',(
        'foreach (ClientUserCurrency currency in pair.Value)',
        'CurrencyItem entry = new CurrencyItem',
    )),
    ('GuildBox','Client/Scenes/Views/GuildDialog.cs','GuildCastlePanel',(
        'foreach (CastleInfo castle in CEnvir.CastleInfoList.Binding)',
        'CastlePanels[castle] = new GuildCastlePanel',
    )),
    ('NPCGoodsBox','Client/Scenes/Views/NPCDialog.cs','NPCGoodsCell',(
        'foreach (NPCGood good in goods)',
        'Cells.Add(cell = new NPCGoodsCell',
    )),
    ('NPCRefineRetrieveBox','Client/Scenes/Views/NPCDialog.cs','NPCRefineCell',(
        'foreach (ClientRefineInfo refine in Refines)',
        'Cells.Add(cell = new NPCRefineCell',
    )),
    ('CharacterBox','Client/Scenes/Views/CharacterDialog.cs','DisciplineMagicCell',(
        'foreach (MagicInfo magic in discipline.Magics)',
        'new DisciplineMagicCell',
    )),
)
...
def main():
    ...
    spec=json.loads(a.spec.read_text(encoding='utf-8'));by={w.get('field'):w for w in spec.get('windows',[])};rows=[];failures=[]
    cache={}
    for field,relative,type_name,evidence in CASES:
        w=by.get(field)
        if w is None:
            failures.append(f'{field} missing for runtime-only {type_name}');continue
        ...
        missing=[needle for needle in evidence if needle not in source]
    ...
    # Existing dedicated boundaries remain authoritative for their wider state.
    ...
    report={'passed':not failures,'runtimeOnlyFamilies':len(CASES),'precreatedRuntimeInstances':0 if not failures else None,'sourceEvidenceChecked':True,'controlsFabricatedByAudit':False,'runtimePayloadsInvented':False,'rows':rows,'failures':failures}
    spec['dynamicCustomRowBoundaryAudit']=report;a.spec.write_text(json.dumps(spec,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
