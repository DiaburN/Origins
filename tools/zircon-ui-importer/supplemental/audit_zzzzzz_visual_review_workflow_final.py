...
def main():
 ...
 expected={'passed':True,'exactShaBuildRequired':True,'exactShaBrowserQaRequired':True,'expectedWindows':80,'expectedScreenshots':80,'domTargetValidation':True,'offlineIndex':True,'concurrencyCancelsStaleRuns':True,'sourceFloor':2511,'nestedFloor':143,'runtimePayloadsInvented':False,'gameWindowContentInvented':False}
 for key,value in expected.items():
  if workflow.get(key)!=value:fail.append(f'visual evidence workflow drifted: {key}={workflow.get(key)!r}, expected {value!r}')
 final['visualReviewWorkflowPassed']=workflow.get('passed') is True;final['visualReviewExpectedScreenshots']=workflow.get('expectedScreenshots');final['visualReviewRequiresExactShaBrowserQa']=workflow.get('exactShaBrowserQaRequired');final['visualReviewRequiresExactShaBuild']=workflow.get('exactShaBuildRequired');final['passed']=final.get('passed') is True and not fail;final['failures']=list(final.get('failures') or [])+fail;spec['finalSupplementalSourceMatrix']=final
 ...
 print('Final visual evidence workflow: PASS -> exact SHA build + Browser QA + 80 validated screenshots')
