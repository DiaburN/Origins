...
def main():
    ...
    for needle,label in (
        ...
        ('StorageSize = (int)StorageTextBox.Value;','storage value binding'),('MemberLimit = (int)MemberTextBox.Value;','member value binding'),('CreateButton.Enabled = CanCreate;','dynamic create gate'),('public bool CanCreate => !CreateAttempted && GuildNameValid && GameScene.Game != null && TotalCost <= GameScene.Game.User.Gold.Amount;','create source gate'),
    ...
    w['guildSourceAudit']={'passed':True,'neutralGuildInfo':None,'initialTab':'CreateTab','initialBackground':266,'creationCost':7500000,'hornInitialChecked':False,'hornInitialCheckedSource':'implicit C# bool default','memberLimitInitial':0,'storageSizeInitial':0,'numericDefaultsSource':'implicit C# int default + DXNumberTextBox default Value','runtimeGuildDataInvented':False,'serverActionsExecutedByReference':False}
    a.spec.write_text(json.dumps(spec,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
    print('Guild source contract: PASS (7 tabs, no-guild CreateTab/#266, implicit Horn/member/storage defaults, 7.5m Gold base cost)')
