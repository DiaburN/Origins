...
source-backed patterns: SizeChanged/helper methods, tab auto-layout, intentional
Point.Empty children, runtime rows/maps and detached nested windows.

This auditor never invents a coordinate. It records source evidence and leaves an
UNKNOWN classification whenever the source cannot yet justify the missing Location.
"""
...
def classify(window: dict, control: dict, text: str) -> dict:
    ...
    if assignments:
        ...
        return {
            ...
            'sourceName':name,'sourceAssignments':assignments[:12],'assignmentCount':len(assignments),
            'runtimeCoordinateInvented':False,
        }

    if ctype in {'DXTab','DXConfigTab'}:
        return {
            'classification':'SOURCE_TAB_AUTO_LAYOUT','sourceName':name,
            'sourceRule':'DXTabControl/DXTab selected-tab layout; content can begin at the tab client origin when no Location is specified',
            'runtimeCoordinateInvented':False,
        }

    if window.get('field')=='ConfigBox' and name=='KeyBindWindow':
        return {
            'classification':'DETACHED_NESTED_WINDOW','sourceName':name,
            ...
            'nestedSourceClass':'DXKeyBindWindow','runtimeCoordinateInvented':False,
        }

    if name=='CharacterNameLabel' and parent=='namePanel':
        return {
            'classification':'SOURCE_DEFAULT_ORIGIN','sourceName':name,
            'sourceRule':'CharacterDialog namePanel is explicitly positioned/sized; CharacterNameLabel intentionally starts at Point.Empty and following guild labels offset from its height',
            'runtimeCoordinateInvented':False,
        }
    ...
    if any(marker in name.lower() for marker in runtime_markers) or parent=='pair.Value':
        return {
            'classification':'RUNTIME_DATA_LAYOUT','sourceName':name,
    ...
    if control.get('syntheticSourceControl'):
        return {
            'classification':'SOURCE_COMPOSITE_LAYOUT','sourceName':name,
            'sourceRule':str(control.get('syntheticSourceControl')),'runtimeCoordinateInvented':False,
        }

    return {
        'classification':'UNKNOWN','sourceName':name,'parent':parent,'type':ctype,'runtimeCoordinateInvented':False,
    }
...
def audit_owner(owner: dict, zircon_root: Path) -> list[dict]:
    ...
    for index,control in enumerate(owner.get('controls',[])):
        ...
        rows.append({
            'window':owner.get('field'),'windowClass':owner.get('class'),'controlIndex':index,
            'control':control.get('name'),'type':control.get('type'),**result,
        })
    return rows
...
def apply(spec: dict, zircon_root: Path) -> dict:
    ...
    for owner in list(spec.get('windows',[]))+list(spec.get('nestedWindows',[])):
        rows.extend(audit_owner(owner,zircon_root))
    ...
    report={
        ...
        'unknownCount':len(unknown),'unknown':unknown,'byWindow':dict(by_window),
...
def main() -> None:
    ...
    print('Controls without Location:',report['totalControlsWithoutConstructorLocation'])
    print('Unplaced classifications:',report['classificationCounts'])
    print('UNKNOWN unplaced controls:',report['unknownCount'])
    for row in report['unknown'][:100]: print('  UNKNOWN',row['window'],row['control'],row['type'],'parent=',row.get('parent'))
    if args.fail_unknown and report['unknownCount']:
        raise SystemExit(f"Unclassified controls without Location: {report['unknownCount']}")
