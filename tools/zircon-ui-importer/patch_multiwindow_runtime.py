...
app_layout_path = Path(sys.argv[1]).resolve()
build_root = app_layout_path.parent
...
def augment_inherited_window_chrome() -> None:
    ...
    if not zircon_root.exists():
        print("Zircon checkout unavailable for inherited DXWindow chrome pass"); return
    spec = json.loads(spec_path.read_text(encoding="utf-8")); changed = 0; titles = 0
    keys = {
    ...
    for item in [*(spec.get("windows") or []), *(spec.get("nestedWindows") or [])]:
        source_path = item.get("sourcePath"); class_name = item.get("class") or item.get("sourceClass")
        ...
        for (child, prop), output_key in keys.items():
            ...
            if root.get(output_key) != value: root[output_key] = value; changed += 1
        title_matches = list(re.finditer(r"\bTitleLabel\s*\.\s*Text\s*=\s*([^;\n]+)\s*;", body))
        if title_matches:
            expression = " ".join(title_matches[-1].group(1).split())
            if root.get("TitleLabelTextExpression") != expression:
                root["TitleLabelTextExpression"] = expression; changed += 1; titles += 1
    by_field = {item.get("field"): item for item in spec.get("windows", [])}
    expected = {"BeltBox": ("false", "false"), "MiniMapBox": ("false", None), "BuffBox": ("false", "false")}
    for field, (close_visible, title_visible) in expected.items():
        root = (by_field.get(field) or {}).get("root", {})
        if root.get("CloseButtonVisible") != close_visible: raise SystemExit(f"Inherited CloseButton.Visible extraction failed for {field}: {root}")
        if title_visible is not None and root.get("TitleLabelVisible") != title_visible: raise SystemExit(f"Inherited TitleLabel.Visible extraction failed for {field}: {root}")
    auto_potion = (by_field.get("AutoPotionBox") or {}).get("root", {})
    if "AutoPotionTitle" not in str(auto_potion.get("TitleLabelTextExpression", "")):
        raise SystemExit(f"Inherited TitleLabel.Text extraction failed for AutoPotionBox: {auto_potion}")
    report = spec.setdefault("inheritedWindowChromePass", {})
    report.update({"assignmentsPromoted": changed, "titleExpressionsPromoted": titles, "source": "derived constructor CloseButton/TitleLabel assignments"})
    spec_path.write_text(json.dumps(spec, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print("Inherited DXWindow chrome assignments promoted:", changed, "titles:", titles)


def neutralize_handcrafted_desktop() -> None:
    text = app_layout_path.read_text(encoding="utf-8")
    pattern = re.compile(r"function buildDesktop\(\) \{.*?\n\}\n\nfunction addImageWindow", re.S)
    replacement = "function buildDesktop() {\n  stage.innerHTML = '';\n  stage.dataset.sourceNeutralDesktop = 'source-default HUD runtime';\n}\n\nfunction addImageWindow"
    text, count = pattern.subn(replacement, text, count=1)
    if count != 1: raise SystemExit("Unable to neutralize legacy buildDesktop() sample")
    forbidden = ["Wizard", "Welcome to ORIGINS", "Zuma Temple", "MiniMap runtime content", "GameScene UI reference"]
    leaked = [value for value in forbidden if value in text]
    if leaked: raise SystemExit(f"Fabricated desktop sample text survived official build: {leaked}")
    app_layout_path.write_text(text, encoding="utf-8")
    print("Legacy handcrafted desktop removed from official viewer")
...
if anchor not in index: raise SystemExit("Animated runtime script anchor missing")
...
index = index.replace(anchor, block, 1); index_path.write_text(index, encoding="utf-8")
